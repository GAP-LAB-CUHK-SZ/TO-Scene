...
class _PointNet2SAModuleBase(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        for i in range(len(self.groupers)):
            new_features = self.mlps[i](features.unsqueeze(-1)).squeeze(-1)  # (B, mlp[-1], N)
            new_features = self.groupers[i](xyz, new_xyz, new_features)  # (B, mlp[-1], npoint, nsample)
            if new_features.size(2) != 1:

...
class PointTransformerSeg(nn.Module):
    def __init__(self, block, blocks, bin_num=2, c=6, k=13):
        super().__init__()
        self.c = c
        ...
        self.bin = nn.Sequential(nn.Linear(bin_num, planes[0]), nn.BatchNorm1d(planes[0]), nn.ReLU(inplace=True))
        self.cls = nn.Sequential(nn.Linear(planes[0]*2, planes[0]), nn.BatchNorm1d(planes[0]), nn.ReLU(inplace=True), nn.Linear(planes[0], k))
    ...
    def forward(self, pxob):
        p0, x0, o0, b0 = pxob  # (n, 3), (n, c), (b)

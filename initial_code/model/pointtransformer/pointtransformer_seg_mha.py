...
class Transformer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        # self.scale = (mid_planes//share_planes) ** -0.5  # 1/sqrt(head_dim)
    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        p_r = p_r.reshape(-1, self.nsample, self.share_planes, self.mid_planes // self.share_planes).contiguous()\
            .permute(0, 2, 3, 1)  # (n, nh, c/nh, nsample)

        x_q = x_q.reshape(-1, self.share_planes, self.mid_planes // self.share_planes).unsqueeze(2) # * self.scale  # (n, nh, 1, c/nh)
        x_k = x_k.reshape(-1, self.nsample, self.share_planes, self.mid_planes // self.share_planes).contiguous()\
            .permute(0, 2, 3, 1)  # (n, nh, c/nh, nsample)

        w = torch.matmul(x_q, x_k + p_r)  # (n, nh, 1, nsample)
        w = self.softmax(w)

        x_v = x_v.reshape(-1, self.nsample, self.share_planes, self.out_planes // self.share_planes).contiguous()\
            .permute(0, 2, 1, 3)  # (n, nh, nsample, c/nh)

        x = torch.matmul(w, x_v).view(-1, self.out_planes)  # (n, nh, 1, c)->(n, c) (:= concat heads)

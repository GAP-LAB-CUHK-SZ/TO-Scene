...
class Transformer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        self.linear_s = nn.Sequential(nn.BatchNorm1d(mid_planes), nn.ReLU(inplace=True),
                                      nn.Linear(mid_planes, mid_planes // share_planes),
                                      nn.BatchNorm1d(mid_planes // share_planes), nn.ReLU(inplace=True),
                                      nn.Linear(mid_planes // share_planes, share_planes))
    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        for i, layer in enumerate(self.linear_s):
            score = layer(score.transpose(1, 2).contiguous()).transpose(1, 2).contiguous() if i % 3 == 0 else layer(score)  # (n, nsample, s)
        score = torch.sigmoid(score) + 0.5

        # import IPython
        # import numpy as np
        # # print(self.linear_s[5].weight)
        # # np.save("./exp/s3dis/pointtransformer_condw_groupscore_g16_temp05/score10.npy", score.cpu().detach().numpy())
        # IPython.embed()

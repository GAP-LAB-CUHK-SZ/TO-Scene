...
class Bottleneck_de(nn.Module):
    ...
    def __init__(self, in_planes, planes, share_planes=8, nsample=16):
        super(Bottleneck_de, self).__init__()
        self.linear1 = nn.Linear(in_planes, planes, bias=False)
        self.bn1 = nn.BatchNorm1d(planes)
        self.transformer2 = Transformer(planes, planes, share_planes, nsample)
        self.bn2 = nn.BatchNorm1d(planes)
...
class PointTransformerSeg(nn.Module):
    def __init__(self, block, block_de, blocks, c=6, k=13):
        super().__init__()
        self.c = c
        ...
        self.dec5 = self._make_dec(block_de, planes[4], 2, share_planes, nsample=nsample[4], is_head=True)  # transform p5
        self.dec4 = self._make_dec(block_de, planes[3], 2, share_planes, nsample=nsample[3])  # fusion p5 and p4
        self.dec3 = self._make_dec(block_de, planes[2], 2, share_planes, nsample=nsample[2])  # fusion p4 and p3
        self.dec2 = self._make_dec(block_de, planes[1], 2, share_planes, nsample=nsample[1])  # fusion p3 and p2
        self.dec1 = self._make_dec(block_de, planes[0], 2, share_planes, nsample=nsample[0])  # fusion p2 and p1
    ...
    def _make_dec(self, block_de, planes, blocks, share_planes=8, nsample=16, is_head=False):
        layers = []
        layers.append(TransitionUp(self.in_planes, None if is_head else planes * block_de.expansion))
        self.in_planes = planes * block_de.expansion
        for _ in range(1, blocks):
            layers.append(block_de(self.in_planes, self.in_planes, share_planes, nsample=nsample))
        return nn.Sequential(*layers)

    def forward(self, pxo):
        p0, x0, o0, no_fps = pxo  # (n, 3), (n, c), (b)
        x0 = p0 if self.c == 3 else torch.cat((p0, x0), 1)
        p1, x1, o1, no_fps = self.enc1([p0, x0, o0, no_fps])
        p2, x2, o2, no_fps = self.enc2([p1, x1, o1, no_fps])
        p3, x3, o3, no_fps = self.enc3([p2, x2, o2, no_fps])
        p4, x4, o4, no_fps = self.enc4([p3, x3, o3, no_fps])
        p5, x5, o5, no_fps = self.enc5([p4, x4, o4, no_fps])
...
def pointtransformer_seg26(**kwargs):
    model = PointTransformerSeg(Bottleneck, Bottleneck_de, [2, 2, 2, 2, 2], **kwargs)
    return model

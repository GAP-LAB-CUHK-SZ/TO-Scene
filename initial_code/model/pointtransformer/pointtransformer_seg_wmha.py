...
class Transformer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        self.linear_w = nn.Sequential(nn.BatchNorm1d(mid_planes // share_planes), nn.ReLU(inplace=True),
    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        w = w.reshape(-1, self.nsample, self.share_planes, self.mid_planes // self.share_planes)\
            .reshape(-1, self.share_planes, self.mid_planes // self.share_planes)   # (n*nsample, s, c//s)

        for i, layer in enumerate(self.linear_w):
            w = layer(w.transpose(1, 2).contiguous()).transpose(1, 2).contiguous() if i % 3 == 0 else layer(w)

        w = w.reshape(-1, self.nsample, self.share_planes, self.mid_planes // self.share_planes)  # (n, nsample, s, c//s)
        w = self.softmax(w)  # (n, nsample, s, c//s)

        n, nsample, c = x_v.shape; s = self.share_planes
        x = ((x_v + p_r).view(n, nsample, s, c // s) * w).sum(1).view(n, c)

...
class PointTransformerSeg(nn.Module):
    def __init__(self, block, blocks, c=6, k=13):
        ...
        self.dec51_bin = self._make_dec_bin(block, planes[4], planes[0], 2, share_planes, nsample=nsample[0])  # fusion p5 and p1
    ...
    def _make_dec_bin(self, block, in_planes, out_planes, blocks, share_planes=8, nsample=16):
        layers = []
        layers.append(TransitionUp(in_planes, out_planes))
        for _ in range(1, blocks):
    ...
    def forward(self, pxo):
        ...
        x5_bin = self.dec51_bin[1:]([p1, self.dec51_bin[0]([p1, x1, o1], [p5, x5, o5]), o1])[1]
        bin_vec = self.bin1(x5_bin)
        ...
        x1 = torch.cat([x1, bin_vec], dim=-1)  # TODO: Try add bin_vec to dec1
        x = self.cls(x1)
        return x, x_bin

...
class Transformer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        self.linear_s = nn.Sequential(nn.Linear(3, 16), nn.BatchNorm1d(16), nn.ReLU(inplace=True),
                                      nn.Linear(3, share_planes))
    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        for i, layer in enumerate(self.linear_s):
            score = layer(score.transpose(1, 2).contiguous()).transpose(1, 2).contiguous() if i == 1 else layer(score)  # (n, nsample, s)
        # score = self.softmax_s(score)
        score = torch.sigmoid(score) + 0.5

        # import IPython
        # import numpy as np
        # # print(self.linear_s[5].weight)
        # # np.save("./exp/s3dis/pointtransformer_cond3d_groupscore_sigmoid/score8.npy", score.cpu().detach().numpy())
        # IPython.embed()

        x = ((x_v + p_r).view(n, nsample, s, c // s) * w.unsqueeze(2))  # n,nsample,s,c//s
        x = score.unsqueeze(-1) * x  # assign scores to each group. n,nsample,s,c//s
        x = x.sum(1).view(n, c)  # n, c
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, share_planes=8, nsample=16):
        ...
        self.bn2 = nn.BatchNorm1d(planes)   # Here channel is c//s
...
class PointTransformerSeg(nn.Module):
    ...
    def _make_enc(self, block, planes, blocks, share_planes=8, stride=1, nsample=16):
        ...
        for _ in range(1, blocks):  # when blocks=2, there's only one block

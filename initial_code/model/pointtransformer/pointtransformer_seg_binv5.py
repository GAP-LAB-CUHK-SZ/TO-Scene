...
from util.data_util import to_categorical
...
class PointTransformerSeg(nn.Module):
    def __init__(self, block, blocks, c=6, k=13):
        ...
        self.enc_bin = self._make_enc_bin(block, c, planes[0], blocks[0], share_planes, stride=stride[0], nsample=nsample[0])  # N/1
        self.bin = nn.Sequential(nn.Linear(planes[0], planes[0], bias=False), nn.BatchNorm1d(planes[0]), nn.ReLU(inplace=True), nn.Linear(planes[0], 2))
        self.conv_bin = nn.Sequential(nn.Linear(2, planes[0]), nn.BatchNorm1d(planes[0]), nn.ReLU(inplace=True))
        ...
        self.cls = nn.Sequential(nn.Linear(planes[0]*2, planes[0], bias=False), nn.BatchNorm1d(planes[0]), nn.ReLU(inplace=True), nn.Linear(planes[0], k))
    ...
    def _make_enc_bin(self, block, in_planes, out_planes, blocks, share_planes=8, stride=1, nsample=16):
        layers = []
        layers.append(TransitionDown(in_planes, out_planes, stride, nsample))
        for _ in range(1, blocks):
            layers.append(block(out_planes, out_planes, share_planes, nsample=nsample))
        return nn.Sequential(*layers)
    ...
    def forward(self, pxo):
        ...
        p_bin, x_bin, o_bin = self.enc_bin([p0, x0, o0])  # TODO: use self.enc1 directly
        x_bin = self.bin(x_bin)  # TODO: use intermediate result of bin1 as bin_vec, instead of max+to_cate+conv_bin 
        bin_vec = x_bin.max(1)[1]
        bin_vec = self.conv_bin(to_categorical(bin_vec, 2))

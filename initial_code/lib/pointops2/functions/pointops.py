...
def queryandgroup(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True):
    ...
    output: new_feat: (m, nsample, c+3), grouped_xyz: (m, nsample, 3)
    """
    ...
    if use_xyz:
        new_features = torch.cat((grouped_xyz_diff, grouped_feat), -1)  # (m, nsample, 3+c)
    else:
...
def interpolation(xyz, new_xyz, feat, offset, new_offset, k=3):
    """
    input: xyz: (m, 3), new_xyz: (n, 3), feat: (m, c), offset: (b of m), new_offset: (b of n)
    output: (n, c)
    """
    assert xyz.is_contiguous() and new_xyz.is_contiguous() and feat.is_contiguous()

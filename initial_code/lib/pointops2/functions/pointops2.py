...
def queryandgroupfdiff(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True):
    """
    input: xyz: (n, 3), new_xyz: (m, 3), feat: (n, c), idx: (m, nsample), offset: (b), new_offset: (b)
    output: new_feat: (m, nsample, c+3)
    """
    ...
    grouped_xyz_diff = grouped_xyz - new_xyz.unsqueeze(1)  # coord_diff: (m, nsample, 3)
    grouped_feat = feat[idx.view(-1).long(), :].view(m, nsample, c)  # (m, nsample, c)
    grouped_feat_diff = grouped_feat - feat.unsqueeze(1)  # coord_diff: (n, nsample, c)
    # grouped_feat = grouping(feat, idx)  # (m, nsample, c)

    if use_xyz:
        return torch.cat((grouped_xyz_diff, grouped_feat_diff), -1)  # (m, nsample, 3+c)
    else:
        return grouped_feat


def queryandgroup_ab(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True):
    """
    input: xyz: (n, 3), new_xyz: (m, 3), feat: (n, c), idx: (m, nsample), offset: (b), new_offset: (b)

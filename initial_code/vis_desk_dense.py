...
def main():
    ...
    names = [line.rstrip('\n') for line in open('data/toscan/toscan_object_names.txt')]
    for i in range(classes):
        color_map[i, :] = get_color(i)
    exp_list = ['pointtransformer_desklevel_dense']
    data_root = '/remote-home/chenpei/data/output/DENSE/process_5e6/npz-desklevel'
    # data_list = sorted(os.listdir(data_root))
    # data_list = [item[:-4] for item in data_list if 'Area_' in item]
    data_list = "data/toscan/val_split_dense.txt"
    ...
    for i in range(len(test_area)):
        result_path = os.path.join('exp/toscan/pointtransformer_desklevel_dense/result')
        ...
        with open(os.path.join(result_path, 'best/pred' + '.pickle'), 'rb') as handle:
            pred = pickle.load(handle)['pred']
        with open(os.path.join(result_path, 'best/label' + '.pickle'), 'rb') as handle:
        ...
        for j in range(len(data_split)):
            ...
            data_path = os.path.join(data_root, data_name + '_dense.npz')
            f = np.load(data_path)

...
def data_prepare(room_path):
    ...
    replace = False if (point_size - labels.size <= labels.size) else True
    point_idxs = range(points.shape[0])
    point_idxs_repeat = np.random.choice(point_idxs, point_size - points.shape[0], replace=replace)
    point_idxs = np.concatenate((point_idxs, point_idxs_repeat))
    np.random.shuffle(point_idxs)
    data_batch = points[point_idxs, :]
    data_batch[:, 0] /= coord_max[0]
    data_batch[:, 1] /= coord_max[1]
    data_batch[:, 2] /= coord_max[2]

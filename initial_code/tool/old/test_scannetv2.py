...
def data_prepare(room_path):
    room_data = torch.load(room_path)
    points, colors, labels = room_data[0], room_data[1], room_data[2]
    assert points.shape[0] == colors.shape[0] == labels.shape[0]
...
def test(model, criterion, names):
    ...
    model.eval()
    val_folder = os.path.join(args.data_root, args.split)
    rooms_split = sorted(os.listdir(val_folder))
    gt_all, pred_all = np.array([]), np.array([])
    check_makedirs(args.save_folder)
    pred_save, gt_save = [], []
    for idx, room_name in enumerate(rooms_split):
        data_room, label_room, index_room, gt = data_prepare(os.path.join(val_folder, room_name))
    ...
    with open(os.path.join(args.save_folder, "gt.pickle"), 'wb') as handle:

...
def main():
    ...
    if args.arch == 'pointnet_seg':
        ...
        from model.pointtransformer.pointtransformer_seg_v80 import pointtransformer_seg26 as Model
...
def data_prepare_detail(points, labels, xyflip=False):
    if xyflip:
        points_xyflip = np.copy(points[:, 0:2])
        points[:, 0] = points_xyflip[:, 1]
        points[:, 1] = points_xyflip[:, 0]
...
def data_prepare(room_path):
    ...
    data_room_1, label_room_1, index_room_1 = data_prepare_detail(points, labels, xyflip=False)
    data_room_2, label_room_2, index_room_2 = data_prepare_detail(points, labels, xyflip=True)
    data_room = np.vstack([data_room_1, data_room_2])
    label_room = np.hstack([label_room_1, label_room_2])
    index_room = np.hstack([index_room_1, index_room_2])

...
def data_prepare(room_path):
    ...
    args.num_point = int(labels.size * args.sample_density)
    num_batch = int(np.ceil(labels.size / args.num_point))
    point_size = int(num_batch * args.num_point)

    point_idxs = np.arange(labels.size)
    point_idxs_repeat = np.random.choice(point_idxs, point_size - point_idxs.size, replace=False)
    point_idxs = np.concatenate((point_idxs, point_idxs_repeat))
    np.random.shuffle(point_idxs)
    data_batch = points[point_idxs, :]
    data_batch[:, 0:3] -= [x_max / 2, y_max / 2, 0]
    data_batch[:, 3:6] /= 255
    label_batch = labels[point_idxs]
    assert np.unique(point_idxs).size == labels.size
    args.batch_size_test = 1
    return data_batch, label_batch, point_idxs, labels

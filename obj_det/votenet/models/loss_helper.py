...
def compute_objectness_loss(end_points):
    ...
    aggregated_vote_xyz = end_points['aggregated_vote_xyz'] # poposal的中心点
    ...
    objectness_mask = torch.zeros((B,K)).to(aggregated_vote_xyz.device)
...
def compute_box_and_sem_cls_loss(end_points, config):
    ...
    objectness_label = end_points['objectness_label'].float().to(object_assignment.device)
    ...
    heading_label_one_hot = torch.cuda.FloatTensor(batch_size, heading_class_label.shape[1], num_heading_bin).zero_().to(pred_center.device)
    ...
    size_label_one_hot = torch.cuda.FloatTensor(batch_size, size_class_label.shape[1], num_size_cluster).zero_().to(pred_center.device)
    ...
    mean_size_arr_expanded = torch.from_numpy(mean_size_arr.astype(np.float32)).to(pred_center.device).unsqueeze(0).unsqueeze(0) # (1,1,num_size_cluster,3) 
...
def get_loss(end_points, config):
    ...
    end_points['pos_ratio'] = \
        torch.sum(objectness_label.float().to(object_assignment.device))/float(total_num_proposal)
    ...
    obj_acc = torch.sum((obj_pred_val==objectness_label.long().to(object_assignment.device)).float()*objectness_mask)/(torch.sum(objectness_mask)+1e-6)

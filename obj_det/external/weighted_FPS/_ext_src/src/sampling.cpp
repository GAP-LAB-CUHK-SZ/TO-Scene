...
at::Tensor furthest_point_sampling(at::Tensor points, const int nsamples, float alpha, float t) {
  CHECK_CONTIGUOUS(points);
  CHECK_IS_FLOAT(points);
  AT_ASSERT(points.size(2)==4, "number of input channels is not 4");
  at::Tensor output =
  ...
  if (points.is_cuda()) {
    furthest_point_sampling_kernel_wrapper(
        points.size(0), points.size(1), nsamples, alpha, t, points.data_ptr<float>(),

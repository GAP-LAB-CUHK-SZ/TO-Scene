...
from utils.sunrgbd_utils import extract_pc_in_box3d
...
class APCalculator(object):
    ...
    def compute_metrics(self):
        ...
        ap_list=list(ap.values())
        new_ap_list=[]
        for item in ap_list:
            if np.isnan(item)==False:
                new_ap_list.append(item)
        ret_dict['mAP'] = np.mean(new_ap_list)
        rec_list = []
        for key in sorted(ap.keys()):
            clsname = self.class2type_map[key] if self.class2type_map else str(key)
            try:
        ...
        ret_dict['AR'] = np.mean(new_rec_list)
        return ret_dict
